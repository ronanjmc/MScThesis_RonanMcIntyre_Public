"""
This is a slightly different version of tudat's mpc.py that handles the
custom_name parameter a bit differently in the to_tudat() function. This is the
version of mpc.py used for the code in this thesis.
"""
...
class BatchMPC:
    ...
    def to_tudat(
        ...
        if "custom_name" in observations_table.columns and observations_table["custom_name"].notna().any():
            unique_link_combos = (
                observations_table.loc[:, ["custom_name", "observatory"]].drop_duplicates()
            ).values
            use_custom_name_for_links = True
        else:
            unique_link_combos = (
                observations_table.loc[:, ["number", "observatory"]].drop_duplicates()
            ).values
            use_custom_name_for_links = False
